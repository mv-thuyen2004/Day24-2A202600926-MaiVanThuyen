    ...
    def test_detection_rate_above_95_percent(self, anonymizer, sample_df):
        ...
        for col in pii_columns:
            for val in sample_df[col].astype(str):
                res = anonymizer.analyzer.analyze(text=val, language="vi", entities=["PERSON", "EMAIL_ADDRESS", "VN_CCCD", "VN_PHONE"])
                if len(res) == 0:
                    failures.append((col, val))
        print(f"\nFailing values count: {len(failures)}")
        print(f"Failing values samples: {failures[:20]}")
